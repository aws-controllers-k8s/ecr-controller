...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.ECRRepositoryPrefix = &identifier.NameOrID

	f3, f3ok := identifier.AdditionalKeys["registryID"]
	if f3ok {
		r.ko.Spec.RegistryID = aws.String(f3)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["ecrRepositoryPrefix"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: ecrRepositoryPrefix"))
	}
	r.ko.Spec.ECRRepositoryPrefix = &primaryKey

	f3, f3ok := fields["registryID"]
	if f3ok {
		r.ko.Spec.RegistryID = aws.String(f3)
	}
